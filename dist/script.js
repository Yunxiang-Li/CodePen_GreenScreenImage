/* initialize four global variables for foreground image, background 
image, canvas1 and canvas2
*/
var fgImage = null;
var bgImage = null;

//get two canvas through their ids
...
/* Function: loadForegroundImage
  Parameter:None
  Return:None
  Does:after user clicks the left upper button,load that file
  change it to a SimpleImage file and draw it to the left canvas
*/
function loadForegroundImage(){
  //output the message
  alert('foreground image loaded');
  //get the uploaded file throught its id
  var imgfile = document.getElementById('fore');
  //transform it into a SimpleImage file
  fgImage = new SimpleImage(imgfile);
  //draw it to the left canvas
...
/* Function: loadBackgroundImage
  Parameter:None
  Return:None
  Does:after user clicks the right upper button,load that file
  change it to a SimpleImage file and draw it to the right canvas
*/
function loadBackgroundImage(){
  //output the message
  alert('background image loaded');
  //get the uploaded file throught its id
  var imgfile = document.getElementById('back');
  //transform it into a SimpleImage file
  bgImage = new SimpleImage(imgfile);
  //draw it to the right canvas
...
/* Function: clearCanvas
  Parameter:None
  Return:None
  Does:after user clicks the right bottom button,clean
  the canvas to default
*/
function clearCanvas(){
  //output the message
  alert('Clear Canvas');
  //get left canvas content
  var ctx1 = canvas1.getContext("2d");
  //clear all in the canvas to default
  ctx1.clearRect(0, 0, canvas1.width, canvas1.height);
  //get right canvas content
...
/* Function: doGreenScreen
  Parameter:None
  Return:None
  Does:after user clicks the left bottom button,create 
  the composite image
*/
function doGreenScreen(){
  //output the message
  alert('Create Composite');
  /*check whether foreground image is uploaded and uploaded completely
  */
  if (fgImage == null || !fgImage.complete()){
    alert('foreground not loaded!');
    return;
  }
  /*check whether background image is uploaded and uploaded completely
  */
  if (bgImage == null || !bgImage.complete()){
    alert('background not loaded!'); 
  }
  //if yes, clear both canvas
  clearCanvas();
  //initialize a variable for the final composite image
  var output = new SimpleImage(fgImage.getWidth(),    fgImage.getHeight());
  //for each pixel of foreground image,get its x and y
  for(var pixel of fgImage.values()){
  var x = pixel.getX();
  var y = pixel.getY();
  /*if pixel's Green value is more than the sum of Red and Blue
  value, use the same position's pixel in background to replace it
  in the same position of final composite image
  */
  if (pixel.getGreen()> (pixel.getRed() + pixel.getBlue())){
     var bgPixel = bgImage.getPixel(x,y);
     output.setPixel(x,y,bgPixel);
  }
  /*else, just use the foreground pixel in the same position of final   composite image
  */
  else{
       output.setPixel(x,y,pixel);
     }
 } 
  //draw the final composite image into the left canvas
